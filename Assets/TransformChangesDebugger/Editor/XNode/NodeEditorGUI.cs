...
namespace TransformChangesDebugger.Editor.XNode {
    ...
    public partial class NodeEditorWindow {
        ...
        private List<global::XNode.Node> culledNodes;
        ...
        protected virtual void OnGUI()
        {
            ...
            Matrix4x4 m = UnityEngine.GUI.matrix;
        ...
        public static void BeginZoomed(Rect rect, float zoom, float topPadding) {
            ...
            padding *= zoom;
            UnityEngine.GUI.BeginClip(new Rect(-((rect.width * zoom) - rect.width) * 0.5f, -(((rect.height * zoom) - rect.height) * 0.5f) + (topPadding * zoom),
                rect.width * zoom,
                rect.height * zoom));
        }
        ...
        public static void EndZoomed(Rect rect, float zoom, float topPadding) {
            ...
            Vector3 offset = new Vector3(
                ...
                0);
            UnityEngine.GUI.matrix = Matrix4x4.TRS(offset, Quaternion.identity, Vector3.one);
        }
        ...
        public void DrawGrid(Rect rect, float zoom, Vector2 panOffset) {
            ...
            UnityEngine.GUI.DrawTextureWithTexCoords(rect, gridTex, new Rect(tileOffset, tileAmount));
            UnityEngine.GUI.DrawTextureWithTexCoords(rect, crossTex, new Rect(tileOffset + new Vector2(0.5f, 0.5f), tileAmount));
        }
        ...
        void ShowPortContextMenu(global::XNode.NodePort hoveredPort) {
            GenericMenu contextMenu = new GenericMenu();
            ...
            foreach (global::XNode.Node node in graph.nodes) {
                ...
                foreach (global::XNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        global::XNode.NodePort input = output.GetConnection(k);
                        ...
                        for (int i = 0; i < reroutePoints.Count; i++) {
                            ...
                            if (selectedReroutes.Contains(rerouteRef)) {
                                UnityEngine.GUI.color = NodeEditorPreferences.GetSettings().highlightColor;
                                UnityEngine.GUI.DrawTexture(rect, NodeEditorResources.dotOuter);
                            }

                            UnityEngine.GUI.color = portColor;
        ...
        private void DrawNodes() {
            ...
            if (Selection.activeObject != null && Selection.activeObject is global::XNode.Node) {
            ...
            List<global::XNode.NodePort> removeEntries = new List<global::XNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<global::XNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                global::XNode.Node node = graph.nodes[n];
                ...
                if (selected) {
                    ...
                    UnityEngine.GUI.color = nodeEditor.GetTint();
        ...
        private bool ShouldBeCulled(global::XNode.Node node) {
