...
using System.Linq;
using TransformChangesDebugger.Editor.XNode.Internal;
using UnityEditor;
using UnityEngine;
...
namespace TransformChangesDebugger.Editor.XNode {
    public partial class NodeEditorWindow {
        ...
        public static global::XNode.Node[] copyBuffer = null;
        ...
        private global::XNode.Node hoveredNode = null;
        [NonSerialized] public global::XNode.NodePort hoveredPort = null;
        [NonSerialized] private global::XNode.NodePort draggedOutput = null;
        [NonSerialized] private global::XNode.NodePort draggedOutputTarget = null;
        [NonSerialized] private global::XNode.NodePort autoConnectOutput = null;
            ...
            switch (e.type) {
                    ...
                    if (e.button == 0) {
                        ...
                        if (currentActivity == NodeActivity.DragNode) {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++) {
                                if (Selection.objects[i] is global::XNode.Node) {
                                    global::XNode.Node node = Selection.objects[i] as global::XNode.Node;
                                    ...
                                    if (offset.sqrMagnitude > 0) {
                                        foreach (global::XNode.NodePort output in node.Outputs) {
                                            Rect rect;
                                        ...
                                        foreach (global::XNode.NodePort input in node.Inputs) {
                                            Rect rect;
                        ...
                        if (IsHoveringPort) {
                            if (hoveredPort.IsOutput) {
                                ...
                                if (hoveredPort.IsConnected) {
                                    global::XNode.Node node = hoveredPort.node;
                                    global::XNode.NodePort output = hoveredPort.Connection;
                    ...
                    if (e.button == 0) {
                        ...
                        if (IsDraggingPort) {
                            ...
                            if (draggedOutputTarget != null && draggedOutput.CanConnectTo(draggedOutputTarget)) {
                                global::XNode.Node node = draggedOutputTarget.node;
                            ...
                            IEnumerable<global::XNode.Node> nodes = Selection.objects.Where(x => x is global::XNode.Node).Select(x => x as global::XNode.Node);
                            foreach (global::XNode.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
                        } else if (!IsHoveringNode) {
                ...
                case EventType.KeyDown:
                    ...
                    if (e.keyCode == KeyCode.A) {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as global::XNode.Node))) {
                            foreach (global::XNode.Node node in graph.nodes) {
                                DeselectNode(node);
                            }
                        } else {
                            foreach (global::XNode.Node node in graph.nodes) {
        ...
        public void Home() {
            var nodes = Selection.objects.Where(o => o is global::XNode.Node).Cast<global::XNode.Node>().ToList();
            if (nodes.Count > 0) {
        ...
        public void RemoveSelectedNodes() {
            ...
            foreach (UnityEngine.Object item in Selection.objects) {
                if (item is global::XNode.Node) {
                    global::XNode.Node node = item as global::XNode.Node;
        ...
        public void RenameSelectedNode() {
            if (Selection.objects.Length == 1 && Selection.activeObject is global::XNode.Node) {
                global::XNode.Node node = Selection.activeObject as global::XNode.Node;
                Vector2 size;
        ...
        public void MoveNodeToTop(global::XNode.Node node) {
            int index;
        ...
        public void DuplicateSelectedNodes() {
            ...
            global::XNode.Node[] selectedNodes = Selection.objects.Select(x => x as global::XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        ...
        public void CopySelectedNodes() {
            copyBuffer = Selection.objects.Select(x => x as global::XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(global::XNode.Node[] nodes, Vector2 topLeft) {
            ...
            Dictionary<global::XNode.Node, global::XNode.Node> substitutes = new Dictionary<global::XNode.Node, global::XNode.Node>();
            for (int i = 0; i < nodes.Length; i++) {
                global::XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;

                // Check if user is allowed to add more of given node type
                global::XNode.Node.DisallowMultipleNodesAttribute disallowAttrib;
                ...
                global::XNode.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++) {
                global::XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (global::XNode.NodePort port in srcNode.Ports) {
                    for (int c = 0; c < port.ConnectionCount; c++) {
                        global::XNode.NodePort inputPort = port.direction == global::XNode.NodePort.IO.Input ? port : port.GetConnection(c);
                        global::XNode.NodePort outputPort = port.direction == global::XNode.NodePort.IO.Output ? port : port.GetConnection(c);

                        global::XNode.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(global::XNode.Node node) {
        ...
        public void AutoConnect(global::XNode.Node node) {
            if (autoConnectOutput == null) return;

            // Find input port of same type
            global::XNode.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && x.ValueType == autoConnectOutput.ValueType);
