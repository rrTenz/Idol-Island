...
namespace TransformChangesDebugger.Editor.XNode {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            global::XNode.Node node = property.serializedObject.targetObject as global::XNode.Node;
            global::XNode.NodePort port = node.GetPort(property.name);
            PropertyField(property, label, port, includeChildren);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, global::XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            PropertyField(property, null, port, includeChildren, options);
        }

        /// <summary> Make a field for a serialized property. Manual node port override. </summary>
        public static void PropertyField(SerializedProperty property, GUIContent label, global::XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            if (property == null) throw new NullReferenceException();
            ...
            if (port == null) EditorGUILayout.PropertyField(property, label, includeChildren, GUILayout.MinWidth(30));
                ...
                if (port.direction == global::XNode.NodePort.IO.Input) {
                    // Get data from [Input] attribute
                    global::XNode.Node.ShowBackingValue showBacking = global::XNode.Node.ShowBackingValue.Unconnected;
                    global::XNode.Node.InputAttribute inputAttribute;
                    ...
                    bool usePropertyAttributes = dynamicPortList ||
                        showBacking == global::XNode.Node.ShowBackingValue.Never ||
                        (showBacking == global::XNode.Node.ShowBackingValue.Unconnected && port.IsConnected);
                    ...
                    foreach (var attr in propertyAttributes) {
                        if (attr is SpaceAttribute) {
                            ...
                            if (usePropertyAttributes) {
                                ...
                                UnityEngine.GUI.Label(position, (attr as HeaderAttribute).header, EditorStyles.boldLabel);
                    ...
                    if (dynamicPortList) {
                        ...
                        global::XNode.Node.ConnectionType connectionType = inputAttribute != null ? inputAttribute.connectionType : global::XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
                        case global::XNode.Node.ShowBackingValue.Unconnected:
                            ...
                            break;
                        case global::XNode.Node.ShowBackingValue.Never:
                            // Display a label
                            EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName));
                            break;
                        case global::XNode.Node.ShowBackingValue.Always:
                ...
                } else if (port.direction == global::XNode.NodePort.IO.Output) {
                    // Get data from [Output] attribute
                    global::XNode.Node.ShowBackingValue showBacking = global::XNode.Node.ShowBackingValue.Unconnected;
                    global::XNode.Node.OutputAttribute outputAttribute;
                    bool dynamicPortList = false;
                    ...
                    if (dynamicPortList) {
                        Type type = GetType(property);
                        global::XNode.Node.ConnectionType connectionType = outputAttribute != null ? outputAttribute.connectionType : global::XNode.Node.ConnectionType.Multiple;
                        DynamicPortList(property.name, type, property.serializedObject, port.direction, connectionType);
                        return;
                    }
                    switch (showBacking) {
        ...
        public static void PortField(global::XNode.NodePort port, params GUILayoutOption[] options) {
            PortField(null, port, options);
        }

        /// <summary> Make a simple port field. </summary>
        public static void PortField(GUIContent label, global::XNode.NodePort port, params GUILayoutOption[] options) {
            if (port == null) return;
        ...
        /// <summary> Make a simple port field. </summary>
        public static void PortField(Vector2 position, global::XNode.NodePort port) {
            if (port == null) return;
        ...
        public static void AddPortField(global::XNode.NodePort port) {
            if (port == null) return;
            Rect rect = new Rect();
        ...
        public static void PortPair(global::XNode.NodePort input, global::XNode.NodePort output) {
        ...
        public static void DrawPortHandle(Rect rect, Color backgroundColor, Color typeColor) {
            ...
            UnityEngine.GUI.color = backgroundColor;
            UnityEngine.GUI.DrawTexture(rect, NodeEditorResources.dotOuter);
            UnityEngine.GUI.color = typeColor;
            UnityEngine.GUI.DrawTexture(rect, NodeEditorResources.dot);
            UnityEngine.GUI.color = col;
        }

#region Obsolete
        [Obsolete("Use IsDynamicPortListPort instead")]
        public static bool IsInstancePortListPort(global::XNode.NodePort port) {
            return IsDynamicPortListPort(port);
        }

        [Obsolete("Use DynamicPortList instead")]
        public static void InstancePortList(string fieldName, Type type, SerializedObject serializedObject, global::XNode.NodePort.IO io, global::XNode.Node.ConnectionType connectionType = global::XNode.Node.ConnectionType.Multiple, global::XNode.Node.TypeConstraint typeConstraint = global::XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
        ...
        public static bool IsDynamicPortListPort(global::XNode.NodePort port) {
        ...
        public static void DynamicPortList(string fieldName, Type type, SerializedObject serializedObject, global::XNode.NodePort.IO io, global::XNode.Node.ConnectionType connectionType = global::XNode.Node.ConnectionType.Multiple, global::XNode.Node.TypeConstraint typeConstraint = global::XNode.Node.TypeConstraint.None, Action<ReorderableList> onCreation = null) {
            global::XNode.Node node = serializedObject.targetObject as global::XNode.Node;
            ...
            var indexedPorts = node.DynamicPorts.Select(x => {
                ...
                return new { index = -1, port = (global::XNode.NodePort) null };
            }).Where(x => x.port != null);
            List<global::XNode.NodePort> dynamicPorts = indexedPorts.OrderBy(x => x.index).Select(x => x.port).ToList();
        ...
        private static ReorderableList CreateReorderableList(string fieldName, List<global::XNode.NodePort> dynamicPorts, SerializedProperty arrayData, Type type, SerializedObject serializedObject, global::XNode.NodePort.IO io, global::XNode.Node.ConnectionType connectionType, global::XNode.Node.TypeConstraint typeConstraint, Action<ReorderableList> onCreation) {
            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    global::XNode.NodePort port = node.GetPort(fieldName + " " + index);
                ...
                (ReorderableList rl) => {
                    ...
                    if (rl.index > reorderableListIndex) {
                        for (int i = reorderableListIndex; i < rl.index; ++i) {
                            global::XNode.NodePort port = node.GetPort(fieldName + " " + i);
                            global::XNode.NodePort nextPort = node.GetPort(fieldName + " " + (i + 1));
            ...
            list.onAddCallback =
                (ReorderableList rl) => {
                    ...
                    if (io == global::XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, global::XNode.Node.TypeConstraint.None, newName);
                    ...
                    if (dynamicPorts[index] == null) {
                        ...
                        for (int k = index + 1; k < dynamicPorts.Count(); k++) {
                            for (int j = 0; j < dynamicPorts[k].ConnectionCount; j++) {
                                global::XNode.NodePort other = dynamicPorts[k].GetConnection(j);
            ...
            if (hasArrayData) {
                ...
                while (dynamicPortCount < arrayData.arraySize) {
                    ...
                    if (io == global::XNode.NodePort.IO.Output) node.AddDynamicOutput(type, connectionType, typeConstraint, newName);
                    else node.AddDynamicInput(type, connectionType, typeConstraint, newName);
                    EditorUtility.SetDirty(node);
                    dynamicPortCount++;
                }
